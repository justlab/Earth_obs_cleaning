...
Borrowed code from `03_import_aeronet.R` on Jdrive  
Import and process the AOD20 V3 level 2 global Aeronet stations  
Interpolating to wavelength AOD 470nm   
...
source("~/Jdrive/PM/Just_Lab/projects/ECHO_PM/code/liuy29/funcs_for_plots.R")
source("~/liuyanguu/ECHO_PM/code/cwv_funcs.R")
...
# load aod data
# aod20_daily_dir <- "/data-coco/ECHO_PM/AeronetAODV3Level2/AOD/AOD20/DAILY/"
aod20_daily_dir <- "/data-coco/ECHO_PM/AeronetAODV3Level2/AOD/AOD20/ALL_POINTS/"
aod20_file_dir  <-  list.files(aod20_daily_dir, pattern = "*.lev20", full.names = TRUE)
...
aod_site_NE <- aod_site_sf[nemia_buff,] 
...
obs_os <- d[, by = obs, .(oneside = (max(exact_wv)-470)*(min(exact_wv)-470)>0)][oneside ==T, obs]
...
### fit the model 
time0 <- Sys.time()
system.time(d2 <- d[, by = obs, .(AOD_470nm = exp(predict(lm(log(aod) ~  log(exact_wv) + I((log(exact_wv))^2)), newdata = data.frame(exact_wv = 470))))])
Sys.time() - time0 # 6.3 hours 6/19
...
# 1 points are set to NA, all of them are in obs_os as well, since there is only one point
length(obs_os) # 91911 
aod20_data_global[N_NAAOD <=4 & obs%in%obs_os,.N] # 91553, so most <=3 are on the same side of 470 nm anyway 
aod20_data_global[N_NAAOD ==3 & obs%in%obs_os,.N]
...
allowed_range <- paste0("AOD_", aod_num[440<=aod_num & aod_num<=500],"nm")
aod20_data_global[obs%in%obs_os &is.na(AOD_440nm)&is.na(AOD_443nm)&is.na(AOD_490nm) &is.na(AOD_500nm), table(N_NAAOD)]
 # exterpolating with 5 or 6 might be fine 
 #    1     2     3     4     5     6 
...
AOD is projected to 470nm, as expected it correlates with neighbouring AOD 440nm
...
# create aod20_NEMIA_idLST, link Aeronet AOT to MAIAC grid (MCD19A2)

* created aod20_NEMIA_idLST that has AERONET Site_Name, idLSTpair0   
...
# Join AOT to MAIAC (MCD19A2)

* rolling join with nearest time (join_time) and by (idLSTpair0)
...
terra0 <- read.fst(lst_files[1], as.data.table = T)
# load lst data for 2018

read.lst <- function(sat = "terra"){
...
vars0 <- c("overpass_index", "Optical_Depth_047", "AOD_Uncertainty", "Column_WV", "AOD_QA",  "RelAZ")
vars_new <- paste0(vars0,"_t")
setnames(aod_lst_terra, vars0, vars_new)
# set NA 
aod_lst_terra[within30min_t==0, c(vars_new):= NA]

# Aqua  ~~~~ #### 
lst_aqua <- read.lst("aqua")
# rolling join
...
aod_lst_aqua[abs(diff_time_min)>30, c(vars_new):= NA]
