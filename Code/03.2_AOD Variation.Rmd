...
aod_conus[,daysN:= uniqueN(date), by = site] # lasting days by site 
...
aod1 <- unique(aod_conus[,.(lat, lon, dailyN, daysN, month, season, 
...
plotAODscatter <- function(aod_data, bySeason = F){
  g1  <-  ggplot(data = aod_data[daysN>=500], aes(x = daysN, y = mediansd))+
      geom_point(size = 1, alpha = 1) + 
      theme_classic() + 
      ggrepel::geom_label_repel(data = aod_data[(daysN>1000&mediansd>0.013)|daysN>1500], aes(x = daysN, y = mediansd, label = site)) + 
...
plotAODmap <- function(aod_data, bySeason = F){
  g2 <- ggplot() +
          geom_polygon(data = map_data("state"), 
                       aes(x = long, y = lat, group = group), 
                       fill = 'NA', color = "grey") + 
          geom_point(data = aod_data[daysN>=500],
          ...
          labs(x = "Longitude", y = "Latitude", color= "Daily\nSD(AOD)", 
               size = "Number of\nDays") +
          ggrepel::geom_label_repel(data = aod_data[daysN>1000&mediansd>0.013], 
...
### Calculating using past 3 years: 2016 - 2019

```{r}
# summary2: limiting the years! aod1 calculates the mean and sd (SD) by day 
aod1 <- unique(aod_conus[year>=2016,.(lat, lon, dailyN, daysN, month, season, 
                            mean_aod = mean(AOD_470nm), sd_aod = sd(AOD_470nm),
                            year = year(date)), by = .(site, date)])
# 290 sites: aod2 [290 * 6]
aod2 <- unique(aod1[,.(mediansd = median(sd_aod), Nobs = sum(dailyN),
