...
dt[ , state.abb := state.abb[match(stringr::str_to_title(ID), state.name)]]
dt[is.na(state.abb), .(ID, Site_Name, geometry)][, table(Site_Name)]
dt[, state.noaa.region := factor(
   ...
   levels = names(noaa.regions))]
# unmatched locations
dt[is.na(state.noaa.region), .(ID, state.abb, Site_Name, geometry)][unique(Site_Name), , mult = "first"]
```
...
*We can customize however we want*

```{r agreement_plot, echo=FALSE}
p1 <- ggplot(dt) +
