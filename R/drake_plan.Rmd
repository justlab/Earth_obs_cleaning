...
nemia_plan <- drake_plan(
  ...
  join = target(rolling_join(mcd_sat = mcd, aer_data_wPred = wPred),
                transform = cross(mcd, wPred)),
  
  # 4. nemia_aer_m
  s = target(aer_in_mcd19(nemia_aer, mcd19 = join),
             transform = map(join)),
  # buffers 
  ref = target(ref_in_buffer(nemia_aer_m = s, refgrid),
             transform = map(s)),
  # sites
  p = target(get_site_list(nemia_aer_m = s), transform = map(s)),
  n = target(get_site_name(nemia_aer_m = s), transform = map(s)),
 
  # mapping
  # this way doesn't work:
  # sub = target(select_refgrid_subset(ref_list0 = ref, point0 = p, name0 = n),
  #              transform = map(ref, p, n))
  sub = target(purrr_map(ref = ref, p = p, n = n),
               transform = map(ref, p, n)),
  # calculate new vars 
  # for terra and aqua, using join, mcd, sub 
  new = target(aod_MODIS_newVars(aod_join_MODIS = join, 
                                 MODIS_all = mcd, refDT_sub = sub),
               transform = map(join, mcd, sub))
)
nemia_plan
...
loadd(cache = ssd_cache)

message("To see one of the aer station with grid around it: ")
knitr::kable(sub_ref_s_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31_p_s_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31_n_s_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31[1:3,])

message("The colocated dataset with new vars")
rmarkdown::paged_table(new_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31_mcd_aqua_sub_ref_s_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31_p_s_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31_n_s_join_mcd_aqua_wPred_nemia_aer_btw_2018.01.01_2018.12.31[1:3,])
```
...
# to see the updated result, run config again
nemia_config <- drake_config(nemia_plan, cache = ssd_cache) # show the dependency
...
nemia_aer <- readd("nemia_aer", cache = ssd_cache)
refgrid <- readd("refgrid", cache = ssd_cache)
