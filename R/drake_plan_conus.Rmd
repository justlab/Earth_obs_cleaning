...
date_start = "2017-01-01" # do not include in sel_data_bytime if don't want to limit
date_end   = "2017-12-31" # do not include in sel_data_bytime if don't want to limit
sats = c("terra", "aqua")
...
conus_cv_config <- drake_config(conus_plan_cv, cache = ssd_cache) # show the dependency
# vis_drake_graph(conus_cv_config, from = names(conus_cv_config$layout))
vis_drake_graph(conus_cv_config)
```
...
make(conus_plan, cache = ssd_cache,
     parallelism = "future", jobs = 20, garbage_collection = TRUE)
Sys.time() - time0
