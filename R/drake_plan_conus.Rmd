...
nemia_plan <- drake_plan(
  ...
  refsub = get_near_cellsid(conus_aer, refgrid),
  ...
  mcd = target(read_mcd19(sat, file_in(mcd19path_CONUS), refsub), transform = map(sat = c("terra", "aqua"))),
  ...
  sel_aer = target(aer_in_mcd19(aer_nearest_2, mcd19 = join),
             transform = map(join)),
  # buffers 
  ref = target(ref_in_buffer(region_aer_m = sel_aer, refgrid),
...
make(nemia_plan, cache = ssd_cache,
     parallelism = "future", jobs = 20, memory_strategy = "speed")
