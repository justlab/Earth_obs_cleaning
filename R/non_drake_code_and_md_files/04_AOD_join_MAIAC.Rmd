...
# but of course, after intersect with the base grid, they won't be circles anymore. 
  ...
  site_list <- st_geometry(nemia_aer_m)
  name_list <- as.list(nemia_aer_m$Site_Name) # for recording site names 
...
refDT_sub_list <- mapply(select.refDT.subset, ref_list, site_list2, name_list)
# # but pmap can work. 
refDT_sub_list <- purrr::pmap(list(ref_list, site_list2, name_list), select.refDT.subset)

# # a loop can work 
for (i in 1:length(ref_list)){
  ref_list0 = ref_list[[i]];  point0 = site_list2[[i]]; name0 = name_list[[i]]
  ref_list0$dist <- sf::st_distance(ref_list0, point0)
  ref_list0$Site_Name <- name0
...
aod_lst_terra <- readRDS("~/Intermediate/AOD_data/190629_LSTjoinMCD19_t_a.rds")$aod_lst_terra
# the original MCD19 
lst_terra <- read_lst(sat = "terra")

# aod_join_MODIS is the rolling-joined file, MODIS_all is like lst_terra
aod.MODIS.newVars <- function(aod_join_MODIS, MODIS_all){
  ...
  setnames(MODIS_all, c("nearest_refgrid", "Optical_Depth_047"), c("idLSTpair0_in_buffer", "AOD_470_buffer"), skip_absent = T)
...
aod_lst_terra <- readRDS("~/Intermediate/AOD_data/190724_LSTjoinMCD19_terra_newVars.rds")
scatter.plot.diagonal(aod_lst_terra, x = "Mean_AOD10km", y = "AOD_470nm")
