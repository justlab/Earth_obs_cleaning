...
We used a copy of the AERONET observations that was last modified `r lubridate::as_date(tz = "UTC", file.mtime(tar_read(aer_orig_obs_path)))`.
...
kbl(pretty.table.numbers(tar_read(cv.summary)[1, -"Year"]), align = "l") %>%
...
The most important difference in our comparison of MAIAC AOD with AERONET observations versus prior validation analyses is the single-pixel comparison (substantially closer spatiotemporal matchups). Here we only include comparisons over CONUS which restricts the range of AOD and aerosol properties.

```{r error_envelope_AMT2018, echo=TRUE}
