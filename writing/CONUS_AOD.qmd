...
After collocation there were \## matched AOD retrievals from \## AERONET sites in CONUS. The median absolute difference from the labeled overpass time (one per 1200-km wide tile) and the time of the matched AERONET observation was \## minutes (25^th^ and 75^th^ percentiles of \## and \## minutes, respectively). While the distribution of AOD is skewed, our estimate of retrieval error and the dependent variable for our predictive model, *diff~AOD~*, was fairly symmetric and Gaussian. 

```{r describe_dv, eval=TRUE}
...
```{r strata_year, eval=TRUE}
kbl(pretty.table.numbers(tar_read(cv.summary)[-1, .SD, .SDcols = -patterns("Q[1-9]")]),
    align = "l") %>%
  kable_styling(position = "left",
                latex_options=c("HOLD_position","scale_down")) %>%
...
kbl(pretty.table.numbers(result[, .SD, .SDcols = -patterns("Time")]), align = "l") %>%
  kable_styling(position = "left",
                latex_options=c("HOLD_position","scale_down")) %>%
  row_spec(0,bold=TRUE) %>% column_spec(1:13, width = "1.1cm")
```

```{r plot_by_month, eval=FALSE, dev='png', dpi=300}
# Color by quantiles
dt[, month := structure(class = "factor",
                        month(time.sat),
                        levels = month.abb)]
...
kbl(pretty.table.numbers(by.region[, .SD, .SDcols = -patterns("Time")]),
    align = "l") %>%
  kable_styling(position = "left",
                latex_options=c("HOLD_position","scale_down")) %>%
...
kbl(pretty.table.numbers(by.highaod[, .SD, .SDcols = -patterns("Time")]),
    align = "l") %>%
  kable_styling(position = "left",
                latex_options=c("HOLD_position","scale_down")) %>%
  row_spec(0,bold=TRUE) %>% column_spec(2:13, width = "1.1cm")
...
### Performance by restriction of matchup calipers

```{r strata_timecalipers, fig.pos="H"}
...
kbl(pretty.table.numbers(by.overpasses[, .SD, .SDcols = -patterns("Time")]),
    align = "l") %>%
  kable_styling(position = "left",
                latex_options=c("HOLD_position","scale_down")) %>%
  row_spec(0,bold=TRUE) %>% column_spec(2:13, width = "1.1cm")
...
kbl(pretty.table.numbers(by.qabest[, .SD, .SDcols = -patterns("Time")]),
    align = "l") %>%
  kable_styling(position = "left",
                latex_options=c("HOLD_position","scale_down")) %>%
  row_spec(0,bold=TRUE) %>% column_spec(1:13, width = "1.1cm")
...
```{r shap_summary, dev='png', dpi=300, eval=FALSE}
...
```{r shap_features, dev='png', dpi=300, eval=FALSE}
...
```{r pred.map.median.mse, eval=FALSE}
pred.map(
...
```{r pred.map.baltimore, eval=FALSE}
pred.map(
