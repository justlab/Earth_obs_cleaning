...
We examined `r l = tar_read(satellite.vs.aqs); scales::comma(l$n.aqs.obs)` observations of AQS PM~2.5~ on `r scales::comma(l$n.cell.times)` cell-days. The correlation of the observations with the original satellite values was `r sprintf("%.03f", l$old)`, compared to `r sprintf("%.03f", l$new)` with our corrected values. In a year-level comparison, against `r l = tar_read(satellite.vs.aqs.yearly); scales::comma(l$n.cell.times)` cell-years of AQS annual means, we found a correlation with the original satellite values of `r sprintf("%.03f", l$old)`, compared to `r sprintf("%.03f", l$new)` with our corrected values.
